                ...
                @if (count($bnetaccount))
                    @foreach ($bnetaccount as $account)
                        @if ($account->winrate && $account->rating && $account->tier)
                            <div>
                                <p class="h2 text-ow-white">WINRATE: {{$account->winrate}} %
                                    RATING: {{$account->rating}}<img
                                            src="{{asset("storage/images/Rank-Icons/" . $account->tier)}}"
                                            alt="{{$account->tier}}" style="
                                            height: 50px; width: auto;">
                                    <?xml version = "1.0" encoding = "utf-8"?>
                                    <!DOCTYPE svg PUBLIC "-//W3C//DTD SVG 1.1//EN"
                                    "http://www.w3.org/Graphics/SVG/1.1/DTD/svg11.dtd">
                                    <svg version="1.1" id="Page-1" xmlns="http://www.w3.org/2000/svg"
                                         xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                                         viewBox="0 0 200.2 188.4" enable-background="new 0 0 200.2 188.4"
                                        ...
                                        <g id="Group_4_" transform="translate(31.067164, 25.895161)">
                                            ...
                                            <path id="Fill-9_3_" fill="#F1960A"
                                                  d="M71.5,9.7V15c16.1,1.2,30.2,9.6,39,22.1l4.3-3.1C105.1,20.2,89.4,10.9,71.5,9.7"/>
                                        ...
                                        <text fill="white" transform="matrix(1 0 0 1 71.4474 129.9033)"
                                              font-family="inherit"
                                              font-size="105px">{{ $account->endorsementlevel }}</text>
                                        </svg>
                                </p>
                            </div>
                        @endif
                        ...
                        <tr>
                            <td>
                                <div class="row">
                                    @foreach($game->heroes as $hero)
                                        <div class="col-lg-4 col-sm-4"><img src="{{ asset($hero->picture) }}"
                                                                            alt="{{ $hero->name }}"
                                                                            class="img-thumbnailtable2 w-100"></div>
                                    @endforeach
                                </div>
                            </td>
                            <td class="text-center">
                                <div class="row">
                                    <div class="col-sm-12 col-lg-12">
                                        <img src="{{ asset($game->map->picture) }}" alt="{{ $game->map->name }}"
                                             class="img-thumbnailtable w-100">
                            ...
                            <td class="pr-0">
                                <form action="/games" method="POST">
                                    ...
                                    <button type="submit" name="delete" value="{{$game->id}}"
