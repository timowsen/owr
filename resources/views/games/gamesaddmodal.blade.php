            ...
            <div class="modal-body">
                <form action="/games" method="POST">
                    ...
                    <div class="fixed-top-add-game mt-1 mr-1">
                        <button type="submit" name="submit" value="1" class="btn btn-xs btn-success">ADD GAME</button>
                        ...
                        <div class="row heroselect">
                            @foreach ($heroes as $hero)
                                <div class="col-md-1 col-4">
                                    <input class="single-checkbox" type="checkbox" name="herochoice[]"
                                           id="{{ $hero->name }}" value="{{ $hero->id }}"/>
                                    <label for="{{ $hero->name }}" class="bla-hm m-0"
                                           style="background-image: url('{{ asset($hero->picture) }}');"></label>
                                </div>
                            @endforeach
                        </div>
                        ...
                        <div class="row">
                            @foreach ($maps as $map)
                                <div class="col-lg-2 col-6">
                                    <figure class="figure">
                                        <input type="radio" name="mapchoice" id="{{ $map->name }}"
                                               value="{{ $map->id }}"/>&nbsp;
                                        <label for="{{ $map->name }}" class="text-ow-white bla-map"
                                               style="background-image: url('{{ asset($map->picture) }}');"></label>&nbsp;
                                        <figcaption class="figure-caption text-center"><b>{{ $map->name }}</b>
                                        </figcaption>
                                    </figure>
                                </div>
                            @endforeach
